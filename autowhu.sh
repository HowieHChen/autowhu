...
if [[ ${KEYWORD} != "" ]]; then
	...
	LOGIN_STATUS=`curl -s 'http://172.19.1.9:8080/eportal/InterFace.do?method=login' -H 'Connection: keep-alive' -H 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.93 Safari/537.36 Edg/96.0.1054.53' -H 'DNT: 1' --data-raw "userId=${USERID}&password=${PASSWORD}&service=Internet&queryString=${QUERY_STRING}&operatorPwd=&operatorUserId=&validcode=&passwordEncrypt=false"`
